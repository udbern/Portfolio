...
export default function Blog() {
  ...
  return (
    <section className='px-5 min-h-[100vh] dark:bg-gray-900 dark:text-gray-100'>
      <h1 className='text-center pt-20 font-semibold text-3xl capitalize pb-10'>welcome to my blog</h1>
                  ...
                  <div className='flex items-end justify-between '>
                    {story.author.image && (
                      <img
                        ...
                        src={urlFor(story.author.image.asset).url()}
                        alt={story.author.image.alt}
                      />
                    )}
                    <span className='text-[15px] md:text-xl'>{new Date(story.publishedAt).toLocaleDateString()}</span>
                  </div>
                  <h2 className='capitalize text-md font-semibold pt-2 '>{story.title}</h2>
                  <p className='pt-1 text-[15px] md:text-xl font-normal'>{`${story.body[0].children[0].text.substring(0, 35)}...`}</p>
